#!/usr/bin/env zunit
#{{{                    MARK:Header
##### Purpose: gh_reveal — plugin-contract pins.
...
@test 'entrypoint stem matches plugin directory basename' {
    ...
    # stem of `gh_reveal.plugin.zsh` must equal the parent directory's
    # basename so generated source lines stay copy-pasteable.
    local entry='gh_reveal.plugin.zsh'
...
@test 'entrypoint parses cleanly under zsh -n' {
    run zsh -n "$pluginDir/gh_reveal.plugin.zsh"
    assert $state equals 0
}
